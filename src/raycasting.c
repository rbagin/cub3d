...
/*   Updated: 2026/01/28 13:57:04 by rbagin        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
{
	ray->map_x = floor(player->pos_x);
	ray->map_y = floor(player->pos_y);
	...
	if (ray->side == 0)
	{
		ray->perp_wall_dist = (ray->map_x - player->pos_x + (1 - ray->step_x) / 2) / player->dir_x;
	}
	else
	{
		ray->perp_wall_dist = (ray->map_y - player->pos_y + (1 - ray->step_y) / 2) / player->dir_y;
	}
}
